...
from btcpy.structs.script import (
    ...
    ScriptSig,
    unhexlify,
)  # ,P2pkhScript,P2shScript,ScriptBuilder,Script
...
def createContractTx(SigScript, PubScript):  # Array of outputs . e.g see outs comment
    ...
    ContractTx = MutableSegWitTransaction(
        ...
        locktime=Locktime(0),
    )  # TODO Failing here .why do we need this ?
    return ContractTx
...
def createContractInput(fundingTx):
    ...
    SigScript = [
        MutableTxIn(
            txid=to_spend,  # Input need not be a segwit transaction.
            txout=0,
            script_sig=Signer,
...
def get_solver(ContractPrivkey):  # todo to fix if & else solvers
    ...
    ELSEsolver = IfElseSolver(Branch.ELSE, TimelockSolver(P2pkhSolver(ContractPrivkey)))
    # TODO How to check/View ??
...
def unsignedTx(intxs, outtxs):
    unsigned = MutableTransaction(
        version=1, ins=intxs, outs=outtxs
    )  # ,locktime=Locktime(0))
    return unsigned
...
def tx_in(tx, inscript):  # TODO change from 1 i/p to multi i/p
    ...
    intxs = [
        TxIn(txid=to_spend.txid, txout=0, sequence=Sequence.max(), script_sig=inscript)
    ]  # ScriptSig.<something>
    return intxs
